          ...
          <div class="card-body">
            <div class="btn-group-vertical w-100">
              <button
                ...
                @click="setExample('192.168.1.0/24')"
              >
                私有網路 Class C (192.168.1.0/24)
              </button>
              <button
                class="btn btn-outline-primary"
                @click="setExample('10.0.0.0/8')"
              >
                私有網路 Class A (10.0.0.0/8)
              </button>
              <button
                class="btn btn-outline-primary"
                @click="setExample('172.16.0.0/12')"
              >
                私有網路 Class B (172.16.0.0/12)
              </button>
              <button
                class="btn btn-outline-primary"
                @click="setExample('127.0.0.0/8')"
              >
                本地回環 (127.0.0.0/8)
              </button>
            </div>
          ...
          <div class="card-body">
            <table class="table table-sm">
              <tbody>
                ...
                <tr>
                  <td><strong>可用 IP 範圍</strong></td>
                  <td>
                    <code>{{ result.usableIpRange }}</code>
                  </td>
                </tr>
                <tr>
            ...
            <table class="table table-sm">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <code>{{ result.firstUsableIp }}</code>
                  </td>
                </tr>
                <tr>
                  <td><strong>最後可用 IP</strong></td>
                  <td>
                    <code>{{ result.lastUsableIp }}</code>
                  </td>
                </tr>
                <tr>
...
export default {
  ...
  methods: {
    ...
    calculateIPv4() {
      ...
      const firstUsableIp =
        totalIps > 1 ? this.intToIP(networkInt + 1) : networkAddress;
      const lastUsableIp =
        totalIps > 1 ? this.intToIP(broadcastInt - 1) : broadcastAddress;
      ...
      this.result = {
        networkAddress,
        broadcastAddress,
        usableIpRange: `${firstUsableIp} - ${lastUsableIp}`,
        subnetMask,
        wildcardMask,
        cidr: `/${cidr}`,
    ...
    generateSubnetSuggestions(currentCidr) {
      ...
      for (
        let cidr = currentCidr + 1;
        cidr <= Math.min(currentCidr + 4, 30);
        cidr++
      ) {
